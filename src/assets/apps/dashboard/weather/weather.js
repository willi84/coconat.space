...
export const SHOW_SUNRISE = false;

export async function loadWeather(showSunrises) {
  const lat = 53.0518;
  const lon = 11.7682;
  ...
  const elementSunTimes = document.getElementById("sonnenzeiten");
  if (elementSunTimes) {
    if(showSunrises) {
        elementSunTimes.innerText = `☀️ Heute: ↑ ${sunriseTimeHeute} ↓ ${sunsetTimeHeute}`;
    } else {
        elementSunTimes.innerText = "";
    }
  }
//   if (showSunrises) {
